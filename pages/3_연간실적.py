...
if not detail_df.empty and '년도' in detail_df.columns:
    year_list = ['전체'] + sorted(detail_df['년도'].dropna().unique().tolist(), reverse=True)
    selected_year = st.selectbox("연도 선택", year_list)
    
    if selected_year != '전체':
        filtered_df = detail_df[detail_df['년도'] == selected_year]
    else:
